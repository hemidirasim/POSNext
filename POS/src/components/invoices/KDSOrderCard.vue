<template>
	<div class="flex-shrink-0 w-80 lg:w-96 bg-white dark:bg-gray-800 rounded-xl shadow-md border overflow-hidden flex flex-col h-full"
		:class="{
			...
			'border-red-500 border-4': order.kds_status === 'Pending' && order.is_recently_modified
	       }">
		...
		<div class="px-4 py-3 border-b flex justify-between items-center"
			...
			<div class="text-right">
				...
				<span v-if="order.kds_status === 'Pending' && order.is_recently_modified" 
					class="text-[10px] uppercase font-bold tracking-wider rounded-full px-2 py-0.5 bg-red-500 text-white mb-1 inline-block">
					{{ __("MODIFIED") }}
				</span>
